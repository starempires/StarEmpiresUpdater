...
public class AddConnectionOrder extends Order {
    ...
    public static AddConnectionOrder parse(final TurnData turnData, final Empire empire, final String parameters) {
        final AddConnectionOrder order = AddConnectionOrder.builder()
                ...
                .gmOnly(OrderType.ADDCONNECTION.isGmOnly())
                .build();
        if (!empire.isGM()) {
        ...
        if (matcher.matches()) {
            ...
            order.addError("Invalid ADDCONNECTION order: " + parameters);
        }
    ...
    public static AddConnectionOrder parseReady(final JsonNode node, final TurnData turnData) {
        ...
        Order.parseReady(node, turnData, OrderType.ADDCONNECTION, builder);
        return builder
